	...
	{  
		...
		if (answer ==1)  
			...
			{  
				...
				location = "?page=duyuru_goruntule&islem=sil&id="+id
...
<?
	...
	$count = mysql_num_rows(mysql_query("select * from duyurular order by tarih desc", $veriyolu));
	...
	$sql_cumlesi="select * from duyurular order by tarih desc LIMIT ".((intval($page)*25)-25).", $perpage";
	...
	if ($sayac < 1)

		{
			...
			echo "Veri Tabanımızda Kayıtlı Duyuru Bulunmamaktadır..."
     ...
     <td width="644" height="17">
       ...
       <table width="668" border="0" align="center" cellpadding="1" cellspacing="1">
         ...
         <tr>
           ...
           <td height="20" colspan="5" align="center" bgcolor="#999999" class="baslikbeyaz">DUYURULARIN LİSTESİ</td>
...
?>
         ...
         <tr>
           ...
           <td width="31" align="center" class="baslikgri12px"> <a href="?page=duyuru_goruntule&islem=goster&id=<?=$satir["id"];?>  "><img src="images/edit.png" width="16" height="16" border="0" title="<?=$satir["baslik"];?> Başlıklı kaydı değiştir."></a></td>
...
if($count > $perpage) : $x = 2; // akrif sayfadan önceki/sonraki sayfa gösterim sayısı 
	...
	if($page==1) 
		...
		echo "<td width=21 height=22><a href=\"?page=duyuru_listele&pagesi=1\">1</a></td>"; 
	...
	for($i; $i<=$page+$x; $i++) 
		...
		{ 
			...
			if($i==$page) 
				...
				echo "<td width=21 height=22  ><a href=\"?page=duyuru_listele&pagesi=$i\">$i</a></td>"; 
	...
	if($page+$x < $lastP-1) 
		...
		{ 
			...
			echo "<td width=21 height=22><a href=\"?page=duyuru_listele&pagesi=$lastP\">$lastP</a></td>"; 
