	...
	{  
		...
		if (answer ==1)  
			...
			{  
				...
				location = "?page=etkinlik_takvimi_listele&sil="+id
...
<?
	...
	if($_GET['sil']):
		...
		$x = mysql_query("DELETE FROM etkinlik_takvimi WHERE id = ".escape($_GET['sil']),$veriyolu) or die(mysql_error());
		
		if($x):
			
			echo '<script>alert("Etkinlik Takvimi Silindi.")</script>';
	...
	$count = mysql_num_rows(mysql_query("select id from etkinlik_takvimi order by tarih desc", $veriyolu));
	...
	$sql_cumlesi="select * from etkinlik_takvimi order by tarih desc LIMIT ".((intval($page)*25)-25).", $perpage";
	...
	if ($sayac < 1)

		{
			...
			echo "Veri Tabanımızda Kayıtlı Etkinlik Takvimi Bulunmamaktadır..."
     ...
     <td width="644" height="17">
       ...
       <table width="668" border="0" align="center" cellpadding="1" cellspacing="1">
         ...
         <tr>
           ...
           <td height="20" colspan="5" align="center" bgcolor="#999999" class="baslikbeyaz">Etkinlik Takvimi Listesi</td>
...
?>
         ...
         <tr>
           ...
           <td width="31" align="center" class="baslikgri12px"> 
           
           <a href="?page=etkinlik_takvimi_ekle&id=<?=$satir["id"];?>">
           
           <img src="images/edit.png" width="16" height="16" border="0" title="<?=$satir["baslik"];?> Başlıklı kaydı değiştir.">
...
if($count > $perpage) : $x = 2; // akrif sayfadan önceki/sonraki sayfa gösterim sayısı 
	...
	if($page==1) 
		...
		echo "<td width=21 height=22><a href=\"?page=etkinlik_takvimi_listele&pagesi=1\">1</a></td>"; 
	...
	for($i; $i<=$page+$x; $i++) 
		...
		{ 
			...
			if($i==$page) 
				...
				echo "<td width=21 height=22  ><a href=\"?page=etkinlik_takvimi_listele&pagesi=$i\">$i</a></td>"; 
	...
	if($page+$x < $lastP-1) 
		...
		{ 
			...
			echo "<td width=21 height=22><a href=\"?page=etkinlik_takvimi_listele&pagesi=$lastP\">$lastP</a></td>"; 
