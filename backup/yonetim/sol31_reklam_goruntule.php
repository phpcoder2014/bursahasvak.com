...
<? 
	...
	if ($_GET["id"] != "" and !ctype_digit($_GET["id"]))
...

		{
			...
			$strmsg3 = "Sol Reklam Güncellendi.";

			$strmsg4 = "Güncelleme sırasında hata oluştu... Girilen bilgiler kaydedilemedi. !";

			$strmsg5 = "Sol Reklam Silindi";

			$strmsg6 = "Silme sırasında hata oluştu... Sol Reklam Silinemedi !";
						 	...
						 	$sql_cumlesi = "update sol3_reklam set baslik='".$tnm_baslik."' where id=".$_POST["id"];
									...
									if ($query_sonucu==true)

										{
...

												{
													...
													if ( strtolower($yuklenen_dosya_uzantisi) == strtolower("jpg") || strtolower($yuklenen_dosya_uzantisi) == strtolower("png") || strtolower($yuklenen_dosya_uzantisi) == strtolower("gif") )
...

														{
															...
															echo "<script>self.location = \"index.php?page=sol31_reklam_goruntule&islem=goster&mesaj=Yüklemek istediğiniz dosya ".$uzanti." uzantılı olduğu için yükleme yapılamaz.\"</script>";	
											...
											echo "<script>self.location = \"index.php?page=sol31_reklam_goruntule&islem=goster&mesaj=".$strmsg3."\"</script>";
...

										{
											...
											echo "<script>self.location = \"index.php?page=sol31_reklam_goruntule&islem=goster&mesaj=".$strmsg4."\"</script>";	
...

					{
						...
						if ($silme_sonucu==true)

							{
								...
								echo "<script>self.location = \"index.php?page=sol31_reklam_goruntule&islem=goster&mesaj=".$strmsg5."\"</script>";
...

							{
								...
								echo "<script>self.location = \"index.php?page=sol31_reklam_goruntule&islem=goster&mesaj=".$strmsg6."\"</script>";
			...
			elseif($_GET["islem"]=="degistir1")

					{
						...
						echo "<script>self.location = \"index.php?page=sol31_reklam_goruntule&islem=goster&mesaj=Swf Dosya Silinmiştir...\"</script>";
			...
			elseif($_GET["islem"]=="degistir2")

					{
						...
						echo "<script>self.location = \"index.php?page=sol31_reklam_goruntule&islem=goster&mesaj=Script Silinmiştir...\"</script>";
...
<form action="index.php?page=sol31_reklam_goruntule&islem=kaydet" method="post" enctype="multipart/form-data" name="frmsol3_reklamgoruntule">
  ...
  <tr>
    ...
    <td width="857"><table width="771" border="0" align="center" cellpadding="1" cellspacing="1">
      ...
      <tr>
        ...
        <td><input name="swf" type="file" id="swf" size="46" /><? if($kayit_satiri["swf"]!==""){echo '&nbsp;&nbsp;<a target="_blank" href="../swf/'.$kayit_satiri["swf"].'">'.$kayit_satiri["swf"].'</a>';}?><input name="eski_swf" value="<?=$kayit_satiri["swf"];?>" type="hidden" />          <br />          <span class="baslikgri12px">Not: Resim Boyutu : w=127 px  h= 179 px</span></td>
...
AC_FL_RunContent( 'codebase','http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=9,0,28,0','src','../swf/<?=$kayit_satiri["swf"]?>','quality','high','pluginspage','http://www.adobe.com/shockwave/download/download.cgi?P1_Prod_Version=ShockwaveFlash','movie','../swf/<?=$kayit_satiri["swf"]?>' ); //end AC code
        ...
        <a href="?page=sol31_reklam_goruntule&amp;islem=degistir1" class="hatamesaji">[   Sil ]</a></td>
