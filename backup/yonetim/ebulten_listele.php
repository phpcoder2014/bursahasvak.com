...
<?
	...
	$count = mysql_num_rows(mysql_query("select * from ebulten_log order by tarih desc", $veriyolu));
	...
	$sql_cumlesi="select * from ebulten_log order by tarih desc LIMIT ".((intval($page)*25)-25).", $perpage";
     ...
     <td width="644" height="17">
       ...
       <table width="668" border="0" align="center" cellpadding="1" cellspacing="1">
         ...
         <tr>
           ...
           <td height="20" colspan="5" align="center" bgcolor="#999999" class="baslikbeyaz">E-BÜLTEN LİSTESİ</td>
         ...
         <tr>
           ...
           <td height="25" colspan="1" bgcolor="#000099" class="baslikbeyaz" >&nbsp;&nbsp; İşlemler</td>
...
?>
         ...
         <tr>
           ...
           <td class="baslikgri12px" style="text-align:center;">

             <? if($satir["tarih"] != "0000-00-00") { ?>

             <?=date("d-m-Y H:i:s",strtotime($satir["tarih"])); } else {?>

           <?='Girilmemiş.'; }?></td>

           <td class="baslikgri12px">&nbsp;&nbsp; <?=$satir["baslik"];?></td>

           <td width="31" align="center" class="baslikgri12px"> <a href="?page=ebulten_goruntule&islem=goster&id=<?=$satir["id"];?>  "><img src="images/edit.png" width="16" height="16" border="0" title="<?=$satir["baslik"];?> Başlıklı kaydı incele."></a></td>
...
if($count > $perpage) : $x = 2; // akrif sayfadan önceki/sonraki sayfa gösterim sayısı 
	...
	if($page==1) 
		...
		echo "<td width=21 height=22><a href=\"?page=ebulten_listele&pagesi=1\">1</a></td>"; 
	...
	for($i; $i<=$page+$x; $i++) 
		...
		{ 
			...
			if($i==$page) 
				...
				echo "<td width=21 height=22  ><a href=\"?page=ebulten_listele&pagesi=$i\">$i</a></td>"; 
	...
	if($page+$x < $lastP-1) 
		...
		{ 
			...
			echo "<td width=21 height=22><a href=\"?page=ebulten_listele&pagesi=$lastP\">$lastP</a></td>"; 
