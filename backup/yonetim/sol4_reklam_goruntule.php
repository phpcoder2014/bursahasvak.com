...
<? 
	...
	if ($_GET["id"] != "" and !ctype_digit($_GET["id"]))
...

		{
			...
			$strmsg3 = "Sol 4 Reklam Güncellendi.";

			$strmsg4 = "Güncelleme sırasında hata oluştu... Girilen bilgiler kaydedilemedi. !";

			$strmsg5 = "Sol 4 Reklam Silindi";

			$strmsg6 = "Silme sırasında hata oluştu... Sol 4 Reklam Silinemedi !";
			...
			if ($_GET["islem"]=="kaydet")

				{
					...
					$tmp_baslik = ucwords(strtolower($tnm_baslik));
						 	...
						 	$sql_cumlesi = "update sol4_reklam set baslik='".$tnm_baslik."' where id=".$_POST["id"];
									...
									if ($query_sonucu==true)

										{
...

												{
													...
													if ( strtolower($yuklenen_dosya_uzantisi) == strtolower("jpg") || strtolower($yuklenen_dosya_uzantisi) == strtolower("png") || strtolower($yuklenen_dosya_uzantisi) == strtolower("gif") )

														{
															...
															$vte = mysql_query("UPDATE sol4_reklam SET swf='".$vtdosyaadi."' WHERE id='".$_POST["id"]."'",$veriyolu);
...

														{
															...
															echo "<script>self.location = \"index.php?page=sol4_reklam_goruntule&islem=goster&mesaj=Yüklemek istediğiniz dosya ".$uzanti." uzantılı olduğu için yükleme yapılamaz.\"</script>";	
											...
											echo "<script>self.location = \"index.php?page=sol4_reklam_listele&islem=goster&mesaj=".$strmsg3."\"</script>";
...

										{
											...
											echo "<script>self.location = \"index.php?page=sol4_reklam_goruntule&islem=goster&mesaj=".$strmsg4."\"</script>";	
			...
			elseif($_GET["islem"]=="sil")

					{
						...
						$sql= @mysql_query("select * from sol4_reklam where id=".$gelenid,$veriyolu);
						...
						$silme_sonucu= mysql_query("delete from sol4_reklam where id=".$gelenid,$veriyolu);
						...
						if ($silme_sonucu==true)

							{
								...
								echo "<script>self.location = \"index.php?page=sol4_reklam_goruntule&islem=goster&mesaj=".$strmsg5."\"</script>";
...

							{
								...
								echo "<script>self.location = \"index.php?page=sol4_reklam_goruntule&islem=goster&mesaj=".$strmsg6."\"</script>";
			...
			elseif($_GET["islem"]=="degistir1")

					{
						...
						$sql_cumlesi = "update sol4_reklam set swf='' where id='1'";

						$query_sonucu = mysql_query($sql_cumlesi, $veriyolu);

						echo "<script>self.location = \"index.php?page=sol4_reklam_goruntule&islem=goster&mesaj=Swf Dosya Silinmiştir...\"</script>";
			...
			elseif($_GET["islem"]=="degistir2")

					{
						...
						$sql_cumlesi = "update sol4_reklam set baslik='' where id='1'";

						$query_sonucu = mysql_query($sql_cumlesi, $veriyolu);

						echo "<script>self.location = \"index.php?page=sol4_reklam_goruntule&islem=goster&mesaj=Script Silinmiştir...\"</script>";
			...
			elseif($_GET["islem"]=="goster")

					{
						...
						$swf_tablosu = @mysql_query("select * from sol4_reklam where id=".$gelenid,$veriyolu);
...
<form action="index.php?page=sol4_reklam_goruntule&islem=kaydet" method="post" enctype="multipart/form-data" name="frmsol4_reklamgoruntule">
  ...
  <tr>
    ...
    <td width="857"><table width="771" border="0" align="center" cellpadding="1" cellspacing="1">
      ...
      <tr>
        ...
        <td height="20" colspan="4" align="center" bgcolor="#999999" class="baslikbeyaz">SOL  ALAN 4 REKLAMI D&Uuml;ZENLE</td>
