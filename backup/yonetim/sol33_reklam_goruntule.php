...

		{
						 	...
						 	$sql_cumlesi = "update sol3_reklam set baslik='".$tnm_baslik."' where id=".$_POST["id"];
									...
									if ($query_sonucu==true)

										{
...

												{
													...
													if ( strtolower($yuklenen_dosya_uzantisi) == strtolower("jpg") || strtolower($yuklenen_dosya_uzantisi) == strtolower("png") || strtolower($yuklenen_dosya_uzantisi) == strtolower("gif") )
...

														{
															...
															echo "<script>self.location = \"index.php?page=sol33_reklam_goruntule&islem=goster&mesaj=Yüklemek istediğiniz dosya ".$uzanti." uzantılı olduğu için yükleme yapılamaz.\"</script>";	
											...
											echo "<script>self.location = \"index.php?page=sol33_reklam_goruntule&islem=goster&mesaj=".$strmsg3."\"</script>";
...

										{
											...
											echo "<script>self.location = \"index.php?page=sol33_reklam_goruntule&islem=goster&mesaj=".$strmsg4."\"</script>";	
...

					{
						...
						if ($silme_sonucu==true)

							{
								...
								echo "<script>self.location = \"index.php?page=sol33_reklam_goruntule&islem=goster&mesaj=".$strmsg5."\"</script>";
...

							{
								...
								echo "<script>self.location = \"index.php?page=sol33_reklam_goruntule&islem=goster&mesaj=".$strmsg6."\"</script>";
			...
			elseif($_GET["islem"]=="degistir1")

					{
						...
						echo "<script>self.location = \"index.php?page=sol33_reklam_goruntule&islem=goster&mesaj=Swf Dosya Silinmiştir...\"</script>";
			...
			elseif($_GET["islem"]=="degistir2")

					{
						...
						echo "<script>self.location = \"index.php?page=sol33_reklam_goruntule&islem=goster&mesaj=Script Silinmiştir...\"</script>";
...
<form action="index.php?page=sol33_reklam_goruntule&islem=kaydet" method="post" enctype="multipart/form-data" name="frmsol3_reklamgoruntule">
...
AC_FL_RunContent( 'codebase','http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=9,0,28,0','src','../swf/<?=$kayit_satiri["swf"]?>','quality','high','pluginspage','http://www.adobe.com/shockwave/download/download.cgi?P1_Prod_Version=ShockwaveFlash','movie','../swf/<?=$kayit_satiri["swf"]?>' ); //end AC code
        ...
        <a href="?page=sol33_reklam_goruntule&amp;islem=degistir1" class="hatamesaji">[   Sil ]</a></td>
